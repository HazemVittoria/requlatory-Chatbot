...
def detect_intent(query: str) -> str:
    ...
    # Explicit internal intent
    if any(k in q for k in ["sop", "internal", "our procedure", "work instruction"]):
        return "SOP"

    # Explicit regulator intent
    if "fda" in q or "inspection" in q or "sampling" in q or "dqst" in q:
        return "FDA"
    ...
    # Implicit intent
    if "how to" in q or "how do we" in q or "procedure" in q:
        return "SOP"

    if "requirements" in q or "submission" in q:
        return "EMA"
...
def build_corpus(data_dir: Path) -> list[dict]:
    ...
    corpus += ingest_folder(data_dir / "sops", "SOP")
    corpus += ingest_folder(data_dir / "others", "OTHER")
    return corpus
...
def search(corpus: list[dict], query: str, intent: str, top_k: int = 30):
    if intent in {"FDA", "EMA", "ICH", "SOP", "OTHER"}:
        corpus = [c for c in corpus if c["source"] == intent]
...
def sentence_score(sentence: str, query: str, source: str) -> float:
    ...
    if procedural:
        ...
        for kw in ["batch", "analysis", "analytical", "testing", "test", "sample", "sampling",
                   "specification", "results", "certificate", "coa"]:
            if kw in s:
                score += 0.6

        # Source bias for procedural questions
        if source == "SOP":
            score += 3.0
        elif source == "FDA":
            score += 2.2
...
def build_answer(
    ...
    for sim, item in rescored:
        ...
        for sent in split_sentences(text):
            ...
            # Sentence relevance filter (must happen per sentence)
            if sentence_score(s, query, item["source"]) < 1.0:
                continue
...
def main():
    ...
    sop_n = sum(1 for c in corpus if c["source"] == "SOP")
    other_n = sum(1 for c in corpus if c["source"] == "OTHER")
    ...
    print(f"  SOP chunks: {sop_n}")
    print(f"  OTHER chunks: {other_n}")
    ...
    while True:
        ...
        results = search(corpus, query, intent=intent, top_k=40)
        answer, citations = build_answer(query, results)

        print(f"\n[Regulatory intent: {intent}]")
        print("\n" + "=" * 80)
